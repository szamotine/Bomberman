...
void game_logic::validate_bomb_flag_time(player* p) { //player* p
...
void game_logic::bomb_flag_red_bricks(int bomb_i_index, int bomb_j_index) {
	...
	for (int i = 0; (unsigned)i < terrain_pointer->get_red_brick_list().size(); i++)//(auto i = terrain_pointer->get_red_brick_list().begin(); i != terrain_pointer->get_red_brick_list().end(); ++i)
	{
...
void game_logic::remove_flagged_bricks() {
	...
	for (int i = 0; unsigned(i) < terrain_pointer->get_red_brick_list().size(); i++)
	{
		...
		if (rb_pointer->get_flag())
		{
			...
			terrain_pointer->get_red_brick_list().erase(terrain_pointer->get_red_brick_list().begin() + i);
		}
	}
}
